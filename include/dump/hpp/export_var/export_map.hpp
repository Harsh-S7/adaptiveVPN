...
namespace cpp_dump
{
    ...
    namespace _detail
    {
        ...
        {
        public:
            explicit _map_dummy_wrapper(const T& map)
                : _map(map)
            {
            }
        ...
        {
        public:
            explicit _multimap_wrapper(const T& map)
        ...
        private:
            ...
            public:
                ...
                multimap_wrapper_iterator(const T& map, It it)
                    : _map(map)
                    , _it(it)
                {
                }
                ...
                bool operator!=(const multimap_wrapper_iterator& to) const
                {
                    return _it != to._it;
                }
        ...
        {
        public:
            _multimap_value_wrapper(It begin, It end)
                : _begin(begin)
                , _end(end)
            {
            }
        ...
        private:
            ...
            {
            public:
                explicit multimap_value_iterator(It it)
                    : _it(it)
                {
                }
                ...
                bool operator!=(const multimap_value_iterator& to) const noexcept
                {
                    return _it != to._it;
                }
        ...
        template<typename T>
        inline auto export_map(const T& map,
            ...
            -> std::enable_if_t<is_map<T>, std::string>
        {
            ...
            if (current_depth >= max_depth)
                return es::bracket("{ ", current_depth) + es::op("...")
    ...
    if constexpr (is_multimap<T>) {
      ...
      return _map_dummy_wrapper(map);
            ...
            {
                shift_indent = is_multimap<T>
                    || (is_iterable_like<typename T::key_type>
                        && !is_tuple<typename T::key_type>)
                    || (is_iterable_like<typename T::mapped_type>
                        && !is_tuple<typename T::mapped_type>);
            }
            else
            {
                shift_indent = false;
            ...
            if (!shift_indent)
            {
                ...
                for (const auto& [is_ellipsis, it, _] : skipped_map)
                {
                    ...
                    if (is_ellipsis)
                    {
                        ...
                        if (last_line_length + get_length(output)
                    ...
                    if constexpr (is_multimap<T>)
                    {
                        ...
                        std::string key_str =
                            export_var(key,
                                       indent,
                                       last_line_length + get_length(output),
                                       next_depth,
                                       true,
                                       key_command)
                            + es::member(" (" + std::to_string(map.count(key)) + ")")
                            + es::op(": ");
            ...
            for (const auto& [is_ellipsis, it, _] : skipped_map)
            {
                ...
                if constexpr (is_multimap<T>)
                {
                    ...
                    std::string key_str = "\n" + new_indent
                        ...
                        + es::member(" (" + std::to_string(map.count(key)) + ")")
                        + es::op(": ");
                    std::string value_str = export_var(values,
                                                       new_indent,
                                                       get_last_line_length(key_str),
                                                       next_depth,
                                                       false,
                                                       value_command);
