...
namespace cpp_dump
{
    ...
    namespace _detail
    {
        ...
        inline std::string export_es_value_t(const es_value_t& esv,
                                             const std::string& indent,
                                             std::size_t last_line_length,
                                             std::size_t current_depth,
                                             bool fail_on_newline,
                                             const export_command& command)
        {
            std::string type_name = es::identifier("cpp_dump") + es::op("::")
                + es::identifier("es_value_t");

            _p_CPP_DUMP_DEFINE_EXPORT_OBJECT_COMMON1_1;

            auto append_output = [&](const std::string& member_name,
                                     const auto& member) -> void
            {
                ...
                else
                {
                    ...
                    output += _export_es_value_vector(member,
                                                      new_indent,
                                                      get_last_line_length(output),
                                                      next_depth,
                                                      false,
                                                      command);
                }
