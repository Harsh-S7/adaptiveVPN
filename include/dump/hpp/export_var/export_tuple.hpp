...
namespace cpp_dump
{
    ...
    namespace _detail
    {
        ...
        inline auto _export_tuple_in_one_line(const T& tuple,
        ...
        {
            std::string output = export_var(_tuple::get<i>(tuple, 0),
                                            indent,
                                            last_line_length,
                                            next_depth,
                                            true,
        ...
        template<std::size_t i, std::size_t size, typename T>
        inline auto _export_tuple_in_lines(const T& tuple,
                                           const std::string& indent,
                                           std::size_t next_depth,
                                           const export_command& command)
            -> std::enable_if_t<is_tuple<T>, std::string>
        {
            ...
            {
                return output + es::op(",\n") + indent
                    + _export_tuple_in_lines<i + 1, size>(
                           tuple, indent, next_depth, command);
            }
            else
            {
                return output;
        ...
        template<typename T>
        inline auto export_tuple(const T& tuple,
                                 const std::string& indent,
                                 std::size_t last_line_length,
            ...
            -> std::enable_if_t<is_tuple<T>, std::string>
        {
            ...
            else
            {
                ...
                if (!has_newline(output)
                    && last_line_length + get_length(output) <= max_line_width)
                    return output;
                ...
                return es::bracket("(\n", current_depth) + new_indent
                    + _export_tuple_in_lines<0, tuple_size>(
                           tuple, new_indent, next_depth, command)
