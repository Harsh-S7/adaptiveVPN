...
\
            template<typename T> \
            inline auto dangerous_export_object(const T& value, \
                                                const std::string& indent, \
                                                std::size_t last_line_length, \
                                                std::size_t current_depth, \
                                                bool fail_on_newline, \
                                                const export_command& command) \
                -> decltype(_p_CPP_DUMP_EXPAND_VA( \
                                _p_CPP_DUMP_EXPAND_FOR_DANGEROUS_EXPORT_OBJECT, \
                                __VA_ARGS__), \
                            std::string()) \
            { \
...
\
                _p_CPP_DUMP_EXPAND_VA(_p_CPP_DUMP_EXPAND_FOR_DANGEROUS_EXPORT_OBJECT2, \
                                      __VA_ARGS__); \
\
