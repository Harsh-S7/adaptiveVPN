...
namespace cpp_dump
{
    ...
    namespace _detail
    {
        ...
        {
        public:
            explicit _set_dummy_wrapper(const T& set)
                : _set(set)
            {
            }
        ...
        {
        public:
            explicit _multiset_wrapper(const T& set)
                : _begin(set, set.begin())
                , _end(set, set.end())
            {
            }
        ...
        private:
            ...
            public:
                ...
                multiset_wrapper_iterator(const T& set, It it)
                    : _set(set)
                    , _it(it)
                {
                }
                ...
                bool operator!=(const multiset_wrapper_iterator& to) const noexcept
                {
                    return _it != to._it;
                }
        ...
        template<typename T>
        inline auto export_set(const T& set,
      ...
      return _set_dummy_wrapper(set);
            ...
            if (!shift_indent)
            {
                ...
                for (const auto& [is_ellipsis, it, _] : skipped_set)
                {
                    ...
                    if constexpr (is_multiset<T>)
                        elem_str +=
                            es::member(" (" + std::to_string(set.count(elem)) + ")");
