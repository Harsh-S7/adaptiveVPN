    ...
    namespace _detail
    {
        ...
        struct export_command
        {
            friend export_command _map_key(export_command&& command)
            {
                ...
                new_command._map_key_child =
                    std::make_unique<export_command>(std::move(command));
                return new_command;
            }
            ...
            {
                export_command new_command(nullptr);
                new_command._map_value_child =
                    std::make_unique<export_command>(std::move(command));
                return new_command;
            }

            friend export_command _map_key_and_value(export_command&& key,
                                                     export_command&& value)
            {
                export_command new_command(nullptr);
                new_command._map_key_child =
                    std::make_unique<export_command>(std::move(key));
                new_command._map_value_child =
                    std::make_unique<export_command>(std::move(value));
                return new_command;
            }
        ...
        public:
            ...
            explicit export_command(int base,
                                    int digits,
                                    int chunk,
                                    bool space_fill,
            ...
            explicit export_command(
                std::function<std::size_t(
                    std::size_t, const std::function<std::size_t()>&)>&& skip_size_func)
                ...
                , _skip_size_func(c._skip_size_func)
            {
                ...
                if (c._map_key_child)
                    _map_key_child =
                        std::make_unique<export_command>(*c._map_key_child);
                if (c._map_value_child)
                    _map_value_child =
                        std::make_unique<export_command>(*c._map_value_child);
            }
            ...
            export_command& operator<<(export_command&& command) &
            {
                ...
                if (command._int_style)
                    _int_style = command._int_style;

                // in the case of show_index
                if (command._show_index)
                    _show_index = command._show_index;
            ...
            const export_command& next() const
            {
                if (_child)
                {
                    if (_int_style && !_child->_int_style)
                        _child->_int_style = _int_style;
                    if (_show_index)
                        _child->_show_index = _show_index;
                }
                else
                {
                    if (!_int_style && !_show_index)
            ...
            const export_command& next_for_map_key() const
            {
                ...
                {
                    if (_int_style && !_map_key_child->_int_style)
                        _map_key_child->_int_style = _int_style;
                    if (_show_index)
                        _map_key_child->_show_index = _show_index;
            ...
            const export_command& next_for_map_value() const
            {
                ...
                {
                    if (_int_style && !_map_value_child->_int_style)
                        _map_value_child->_int_style = _int_style;
                    if (_show_index)
                        _map_value_child->_show_index = _show_index;
        ...
        private:
            ...
            std::function<std::size_t(std::size_t, const std::function<std::size_t()>&)>
                _skip_size_func;
        ...
        inline const export_command export_command::default_command(
            _default_skip_size_func);
        ...
        public:
            ...
            explicit value_with_command(const T& v, export_command&& c)
                : value(v)
                , command(std::move(c))
            {
            }
            explicit value_with_command(const T& v, const export_command& c)
                : value(v)
                , command(c)
            {
            }
        ...
        template<typename T>
        value_with_command<T> operator|(value_with_command<T>&& vc,
                                        export_command&& command)
        {
            return value_with_command<T>(vc.value,
                                         const_cast<export_command&&>(vc.command)
                                             << std::move(command));
        }
                                        ...
                                        const export_command& command)
        {
            return value_with_command<T>(
                vc.value, const_cast<export_command&&>(vc.command) << command);
        }

        template<typename T>
        value_with_command<T> operator|(const value_with_command<T>& vc,
                                        export_command&& command)
        {
        ...
        inline export_command operator|(const export_command& lhs,
                                        const export_command& rhs)
        {
            return lhs << rhs;
        }
        ...
        template<typename T>
        inline constexpr bool is_value_with_command =
            _is_value_with_command<_remove_cvref<T>>;
 ...
 */
    inline auto int_style(int base                                 = 16,
                          int digits                               = -1,
                          int chunk                                = 2,
 ...
 */
    inline auto back(std::size_t iteration_count = max_iteration_count)
    {
        return _detail::export_command(
            [=](std::size_t index,
                const std::function<std::size_t()>& cont_size) -> std::size_t
            {
                std::size_t size = cont_size();
                std::size_t first =
                    size >= iteration_count ? size - iteration_count : 0;
    ...
    inline auto middle(std::size_t iteration_count = max_iteration_count)
    {
        return _detail::export_command(
                ...
                const std::function<std::size_t()>& cont_size) -> std::size_t
            {
                std::size_t size = cont_size();
                std::size_t first =
                    size >= iteration_count ? (size - iteration_count) / 2 : 0;
 ...
 * See README for details.
 */
    inline auto map_k(_detail::export_command&& c)
    {
        return _map_key(std::move(c));
    }
 ...
 * See README for details.
 */
    inline auto map_v(_detail::export_command&& c)
    {
        return _map_value(std::move(c));
    }
 ...
 */
    inline auto index()
    {
        return _detail::export_command(true);
    }
